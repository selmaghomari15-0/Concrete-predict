...
# Titre
st.markdown('<div class="main-title"> Prédiction de la Résistance du Béton</div>',
            unsafe_allow_html=True)
...
if page == " Prédiction":
    ...
    with col2:
        st.subheader(" Adjuvants et granulats")
    ...
    rapport_ec = eau / ciment if ciment > 0 else 0
    if rapport_ec > 0.7:
        st.warning(f"⚠️ Rapport eau/ciment élevé ({rapport_ec:.2f}) "
                   f"— résistance probablement faible.")
    if ciment < 150:
        st.warning("⚠️ Dosage en ciment très faible — "
                   "formulation hors domaine d'entraînement.")
...
st.markdown(
    ...
    "Application développée par <b>Salma Rhomari</b> — PFE Licence SSD "
